 ...
 * @since 1.0.0
 */
define("quizaccess_cheatdetect/extension-detector/index",["quizaccess_cheatdetect/extension-detector/detector"],function(ExtensionDetector){let detectorInstance=null;function startDetector(){try{return detectorInstance?(console.log("🧩 Extension Detector: Instance already created"),detectorInstance):(detectorInstance=new ExtensionDetector.ExtensionDetector,detectorInstance.start(),console.log("🧩 Extension Detector: Started successfully"),detectorInstance)}catch(error){return console.error("🧩 Extension Detector: Startup error",error),null}}return{init:function(backendParams){try{if("undefined"==typeof MutationObserver)return null;if(window._trackingInitialized)return startDetector();{const checkTracking=setInterval(()=>{window._trackingInitialized&&(clearInterval(checkTracking),startDetector())},100);return setTimeout(()=>{clearInterval(checkTracking),detectorInstance||(console.warn("🧩 Extension Detector: Timeout, forced start without tracking"),startDetector())},5e3),null}}catch(error){return console.error("🧩 Extension Detector: Initialization error",error),null}},getMetrics:function(){if(!detectorInstance)return JSON.stringify({timestamp:(new Date).toISOString(),extensionDetection:{}});try{return detectorInstance.exportMetricsAsJSON()}catch(error){return JSON.stringify({timestamp:(new Date).toISOString(),extensionDetection:{},error:error.message})}}}});
